angular.module('wui.date',[]).directive('wuiDate', function() {
	...
	function fieldTemplate(scope, element, attr) {
		return(
			...
			'<span class="title">' +
			'<span class="txt" ng-click="openYearPicker()" ><span id="date_year">{{date.year}}</span> 年 </span>' +
			'<span class="txt" ng-click="openMonthPicker()" ><span id="date_mouth">{{date.month}}</span> 月</span>' +
			'</span>' +
