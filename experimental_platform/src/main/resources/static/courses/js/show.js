...
function getCourseExperimentalSuccess(data) {
    if(data.indexOf("该课程暂时没有实验内容！") == 0) {
        ...
        for(var i = 0; i < experimentalDocumentList.length; i++) {
            ...
            str += "<a class=\"btn btn-default\" href=\"/courses/show_experimental_score.html?coursesId="+ experimentalDocumentList[i].courseId + "&experimentalId="+experimentalDocumentList[i].id +"\" >成绩查询</a>"
            str += "</div>";
            str += "</div>";
        }
...
function uploadExperimentalReportFormSubmit() {
    ...
    var  id = document.getElementById("uploadStudentIdInput").value;
    var student_name = document.getElementById("uploadStudentNameInput").value;
    if (name == "" || name == null || name.length == 0||id == ""||id == null||id.length == 0||student_name.length ==0 ||student_name == "") {
        alert("实验报告名称不能为空！")
    } else {
        if (name.length > 200||id.length>10) {
...
function uploadExperimentalReportFormSubmitSuccess(data) {
    if (data.indexOf("ok") == 0) {
        ...
        if (data.indexOf("该实验不存在！") == 0) {
            alert(data);
            window.location.href = "/courses/show.html";
        } else {
            alert(data);
        }
    }
...
function uploadExperimentalCheckWork() {
    ...
    obj.mouth = mouth.toString();
    obj.day = day.toString();
    if (studentId == ""||studentId == null||studentName == "" ||studentName == null){
        alert("请输入正确信息！");
        location.href = "/courses/show.html?courseId="+courseId;
    }else {
        $.ajax({
            url: "/upload_check_work",
            type: "post",
            data: obj,
            success: uploadExperimentalCheckWorkSuccess,
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                if (XMLHttpRequest.status >= 400 && XMLHttpRequest.status < 500) {
                    alert("客户端请求出错！错误代码（" + XMLHttpRequest.status + "," + XMLHttpRequest.readyState + "," + textStatus + "）");
                } else {
...
function uploadExperimentalCheckWorkSuccess(data){
    if(data.indexOf("签到成功") != -1||data.indexOf("请勿重复签到") != -1){
        alert(data);
    }
    else {
        alert("发生错误！");
...
function gotoCheckWorkPage(){
    var courseId = window.location.href.split("?")[1].split("=")[1];
    window.location.href="/courses/checkWork.html?courseId="+courseId;
}
