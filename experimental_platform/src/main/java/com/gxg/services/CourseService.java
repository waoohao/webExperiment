...
//                            String result = ftpService.ftpSendFile(path, experimentalEnvironmentUrl, experimentalNode.getIp(), username, password);
    ...
    public String uploadExperimentalReportScore(String id, String score) {
        experimentalReportDao.updateExperimentalReportScoreById(id, score);
    ...
    public String getExperimentalScore(String experimentalId) {
        List<ExperimentalReport> experimentalReportsList = experimentalReportDao.getExperimentalReportByExperimentalId(experimentalId);
        if (experimentalReportsList.isEmpty()) {
            return "没有该实验";
        } else {
            ArrayList<String> experimentalReportArrayList = new ArrayList<>();
            for (int i = 0; i < experimentalReportsList.size(); i = i + 1) {
                JSONObject jsonObject = new JSONObject();
                jsonObject.accumulate("studentId", experimentalReportsList.get(i).getStudentId());
                jsonObject.accumulate("studentName", experimentalReportsList.get(i).getStudentName());
                jsonObject.accumulate("experimentalScore", experimentalReportsList.get(i).getExperimentalScore());
    ...
    public String uploadExperimentalCheckWork(ExperimentalCheckWork experimentalCheckWork) {
        String statu = experimentalCheckWorkDao.insertSignIn(experimentalCheckWork);
        return statu;
    }

    public String getCheckWorkDate(String mouth, String year, String courseId) {
        String dateString = year + "-" + mouth;
        List<ExperimentalCheckWork> list = experimentalCheckWorkDao.getCheckWorkDateByDateString(dateString, courseId);
        List<String> jsonlist = new ArrayList<>();
        for (int i = 0; i < list.size(); i++) {
            jsonlist.add(list.get(i).toString());
        }
    ...
    public String takeScoreToExcel(String coureseId) {
        List<ExperimentalDocument> experimentalDocumentsList = experimentalDocumentDao.getExperimentalDocumentByCourseId(coureseId);
        List<String> experimentalReportList = new ArrayList<>();
        for (int i = 0; i < experimentalDocumentsList.size(); i++) {
            String experimentalId = experimentalDocumentsList.get(i).getId();
            List<ExperimentalReport> l = new ArrayList<>();
            l = experimentalReportDao.getExperimentalReportByExperimentalId(experimentalId);
            for (int j = 0; j < l.size(); j++) {
                if (l.get(j).getExperimentalScore() != null) {
                    experimentalReportList.add(l.get(j).toString());
                }
            }
        }
        ...
        jsonObject.accumulate("courseName",experimentalDocumentsList.get(0).getTitle());
        return jsonObject.toString();
    }
}
